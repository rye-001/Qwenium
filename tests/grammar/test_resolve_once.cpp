...
void test_equivalence_initial_position()
{
    ...
    auto grammar_bf = qinf::GrammarVocab::parse_impl(RESOLVE_ONCE_GRAMMAR);
    ASSERT_NE(grammar_bf, nullptr);
    auto valid_bf = grammar_bf->get_valid_tokens(vocab);

    // Get valid tokens WITH trie
    auto grammar_trie = qinf::GrammarVocab::parse_impl(RESOLVE_ONCE_GRAMMAR);
...
void test_space_literal_many_candidates()
{
    ...
    auto grammar = qinf::GrammarVocab::parse_impl(RESOLVE_ONCE_GRAMMAR);
...
root ::= "await" " " "getAccounts" "(" ")" ";"
    ...
    // Without trie
    auto grammar_bf = qinf::GrammarVocab::parse_impl(grammar_str);
    grammar_bf->accept_token(0, vocab); // "await"
    auto valid_bf = grammar_bf->get_valid_tokens(vocab);

    // With trie
    auto grammar_trie = qinf::GrammarVocab::parse_impl(grammar_str);
    grammar_trie->set_token_trie(&trie);
