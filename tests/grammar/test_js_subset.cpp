// test_js_subset.cpp
// JS-subset grammar integration tests adapted for qinf::GrammarVocab.
//
...
class Acc {
public:
    Acc(qinf::GrammarVocab* g, const std::vector<std::string>& v) : g(g), v(v) {}
...
{
    auto g = qinf::GrammarVocab::parse_impl(JS_SUBSET_GRAMMAR);
    ASSERT_NE(g, nullptr);
}
